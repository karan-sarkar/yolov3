...
def train(hyp, opt, device, tb_writer=None):
    ...
    if opt.adam:
        ...
        c_optimizer = optim.Adam(cg1, lr=hyp['lr0'], weight_decay = hyp['weight_decay'], betas=(hyp['momentum'], 0.999))
    else:
        ...
        c_optimizer = optim.SGD(cg1, weight_decay = hyp['weight_decay'], lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
    ...
    c_scheduler = lr_scheduler.LambdaLR(c_optimizer, lr_lambda=lf)
    ...
    if pretrained:
        ...
        if ckpt['c_optimizer'] is not None:
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(source_train_path, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    target_dataloader, target_dataset = create_dataloader(target_train_path, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    mlc = np.concatenate(dataset.labels, 0)[:, 0].max()  # max label class
    nb = min(len(dataloader), len(target_dataloader))  # number of batches
    assert mlc < nc, 'Label class %g exceeds nc=%g in %s. Possible class labels are 0-%g' % (mlc, nc, opt.data, nc - 1)

    # Process 0
    if rank in [-1, 0]:
        testloader = create_dataloader(source_test_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '))[0]
        target_testloader = create_dataloader(target_test_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '))[0]
    ...
    c_scheduler.last_epoch = start_epoch - 1
    g_scheduler.last_epoch = start_epoch - 1    # do not move
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'total', 'discrep', 'labels', 'img_size'))
        if rank in [-1, 0]:
        ...
        if rank in [-1, 0]:
            ...
            for x, tag in zip(list(mloss[:-2]) + list(target_results) + g_lr + c_lr, tags):
                if tb_writer:
                    tb_writer.add_scalar(tag, x, epoch)  # tensorboard
                if wandb_logger.wandb:
            ...
            # Update best mAP 
            fi = fitness(np.array(target_results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
